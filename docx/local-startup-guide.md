...
> 版本：1.1.0 | 更新日期：2026-06-27 | 命令已于 Python 3.12.6 / Windows 实测通过
...
在根目录新建 `.env`（已在 `.gitignore`，不会被提交）：

```ini
# —— 鉴权 ——
ACG_AI_API_KEY=dev-api-key            # 生产环境务必改为强随机值；未配置时默认 dev-api-key

# —— meta-LLM（提示词生成 generate / 合规校验 moderate 依赖）——
